    ...
    <article className="relative px-4 py-4 border-b border-border hover:bg-secondary/20 transition-all duration-300 cursor-pointer group/post">
      <div className="flex gap-4">
        ...
        <div className="flex flex-col items-center flex-shrink-0 relative">
          <Link href={`/@${author.username}`} className="z-10 bg-background rounded-full relative">
          ...
          {/* Thread line */}
          {showThread && (
            <div className="absolute left-1/2 top-12 bottom-0 w-[2px] bg-border/50 -translate-x-1/2 rounded-full" />
          )}
        </div>
        ...
        <div className="flex-1 min-w-0 pb-1">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-1.5 overflow-hidden">
              ...
              {author.is_verified && (
                <svg aria-label="Verified" fill="#0095F6" height="12" role="img" viewBox="0 0 40 40" width="12" className="flex-shrink-0">
          ...
          {/* Content text */}
          {content && (
            <div className="text-foreground text-[15px] leading-[1.4] whitespace-pre-wrap break-words mt-1">
          ...
          {media && media.length > 0 && (
            <div className={`mt-3 overflow-hidden rounded-xl border border-border/50 max-w-full ${media.length > 1 ? 'grid gap-1 grid-cols-2' : ''}`}>
              {media.map((item) => (
                <div key={item.id} className="relative bg-muted w-full aspect-auto min-h-[100px]">
...
function renderContent(content: string): React.ReactNode {
  ...
  return parts.map((part, index) => {
    if (part.startsWith('#')) {
      ...
      return (
        <Link key={index} href={`/hashtag/${tag}`} className="text-primary hover:underline font-medium">
          {part}
        </Link>
      );
    }
    if (part.startsWith('@')) {
      ...
      return (
        <Link key={index} href={`/@${username}`} className="text-primary hover:underline font-medium">
          {part}
        </Link>
      );
    }
